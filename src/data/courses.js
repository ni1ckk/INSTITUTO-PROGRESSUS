export const coursesData = [
  {
    id: 1,
    imageSrc: "/assets/img/coursesCards/6.png",
    authorImageSrc: "/assets/img/general/avatar-1.png",
    title: "Matemática Financeira",
    rating: 4.3,
    ratingCount: 1991,
    lessonCount: 6,
    duration: 1320,
    level: "Iniciante",
    originalPrice: 199,
    discountedPrice: 79,
    paid: true,
    category: "Matemática",
    state: "Popular",
    authorName: "Diego Rocha",
    viewStatus: "Bom",
    difficulty: "Fácil",
    desc: "Aprenda a calcular juros, parcelas, descontos e entender o dinheiro no dia a dia. Ideal para quem quer tomar decisões financeiras com mais segurança e praticidade.",
  },
  {
    id: 2,
    imageSrc: "/assets/img/coursesCards/9.png",
    authorImageSrc: "/assets/img/general/avatar-1.png",
    title: "Biologia Humana",
    rating: 4.7,
    ratingCount: 1991,
    lessonCount: 6,
    duration: 410,
    level: "Avançado",
    originalPrice: 189,
    discountedPrice: 89,
    paid: true,
    category: "Ciências da Natureza",
    authorName: "Simone Dos Santos",
    popular: true,
    new: true,
    bestSeller: true,
    state: "Em andamento",
    viewStatus: "Baixo",
    difficulty: "Fácil",
    desc: "Entenda os sistemas do corpo humano, saúde, doenças, vacinas e funcionamento do organismo — tudo com foco nas questões mais cobradas no ENEM.",
  },
  {
    id: 3,
    imageSrc: "/assets/img/coursesCards/6.png",
    authorImageSrc: "/assets/img/general/avatar-1.png",
    title: "Matemática Financeira",
    rating: 4.5,
    ratingCount: 1991,
    lessonCount: 6,
    duration: 1220,
    level: "Intermediário",
    originalPrice: 249,
    discountedPrice: 129,
    authorName: "Diego Rocha",
    paid: true,
    category: "Matemática",
    state: "Em alta",
    viewStatus: "Alto",
    difficulty: "Fácil",
    desc: "Aprofunde seus conhecimentos em juros compostos, descontos sucessivos, correção monetária e problemas aplicados ao cotidiano. Ideal para quem já domina o básico.",
  },
  {
    id: 4,
    imageSrc: "/assets/img/coursesCards/1.png",
    authorImageSrc: "/assets/img/general/avatar-1.png",
    title: "Gramática para o dia a dia",
    rating: 4.2,
    ratingCount: 1991,
    lessonCount: 6,
    duration: 1020,
    level: "Avançado",
    originalPrice: 179,
    discountedPrice: 99,
    authorName: "José Silva",
    paid: true,
    category: "Linguagens e Redação",
    state: "Em alta",
    viewStatus: "Alto",
    difficulty: "Fácil",
    desc: "Revise os principais tópicos de gramática exigidos no ENEM de forma prática: pontuação, crase, concordância, regência, uso dos tempos verbais e muito mais.",
  },
  {
    id: 5,
    imageSrc: "/assets/img/coursesCards/11.png",
    authorImageSrc: "/assets/img/general/avatar-1.png",
    title: "Geografia atual",
    rating: 3.8,
    ratingCount: 1991,
    lessonCount: 6,
    duration: 250,
    level: "Intermediário",
    originalPrice: 169,
    discountedPrice: 79,
    paid: true,
    category: "Ciências Humanas",
    authorName: "Marcos Amaral",
    state: "Popular",
    viewStatus: "Baixo",
    difficulty: "Meduium",
    desc: "Estude geopolítica, meio ambiente, urbanização, globalização e mais. Conteúdos atualizados e cobrados frequentemente nas provas de Ciências Humanas.",
  },
  {
    id: 6,
    imageSrc: "/assets/img/coursesCards/2.png",
    imageAlt: "image",
    rating: 4.6,
    ratingCount: 1991,
    title: "Leitura com Propósito",
    lessonCount: 6,
    duration: 800,
    level: "Avançado",
    authorImageSrc: "/assets/img/general/avatar-1.png",
    authorName: "José Silva",
    originalPrice: 209,
    discountedPrice: 99,
    paid: true,
    category: "Linguagens e Redação",
    state: "Em andamento",
    viewStatus: "Alto",
    difficulty: "Média",
    desc: "Aprenda a ler com estratégia, identificar intenções do autor, interpretar diferentes gêneros textuais e resolver questões com mais segurança.",
  },
  {
    id: 7,
    imageSrc: "/assets/img/coursesCards/7.png",
    imageAlt: "image",
    rating: 3.5,
    ratingCount: 1991,
    title: "Física no cotidiano",
    lessonCount: 6,
    duration: 490,
    level: "Intermediário",
    authorImageSrc: "/assets/img/general/avatar-1.png",
    authorName: "Simone Dos Santos",
    originalPrice: 299,
    discountedPrice: 149,
    paid: true,
    category: "Ciências da Natureza",
    state: "Popular",
    viewStatus: "Alto",
    difficulty: "Difícil",
    desc: "Entenda os conceitos físicos por trás de situações reais: movimento, energia, eletricidade, calor e muito mais. Aprenda com exemplos práticos, como o ENEM gosta de cobrar.",
  },
  {
    id: 8,
    imageSrc: "/assets/img/coursesCards/5.png",
    imageAlt: "image",
    rating: 4.8,
    ratingCount: 1991,
    title: "Raciocínio Lógico",
    lessonCount: 6,
    duration: 500,
    level: "Avançado",
    authorImageSrc: "/assets/img/general/avatar-1.png",
    authorName: "Diego Rocha",
    originalPrice: 219,
    discountedPrice: 109,
    paid: true,
    category: "Matemática",
    state: "Em andamento",
    viewStatus: "Médio",
    difficulty: "Média",
    desc: "Domine questões complexas e desenvolva rapidez e precisão para se destacar no ENEM.",
  },
  {
    id: 9,
    imageSrc: "/assets/img/coursesCards/6.png",
    imageAlt: "image",
    rating: 3.9,
    ratingCount: 1991,
    title: "Matemática Financeira",
    lessonCount: 6,
    duration: 920,
    level: "Avançado",
    authorImageSrc: "/assets/img/general/avatar-1.png",
    authorName: "Diego Rocha",
    originalPrice: 199,
    discountedPrice: 89,
    paid: true,
    category: "Matemática",
    state: "Popular",
    viewStatus: "Alto",
    difficulty: "Difícil",
    desc: "Enfrente os desafios dos cálculos financeiros mais exigentes: amortizações, séries de pagamentos, taxas equivalentes, análise de investimentos e questões complexas no estilo ENEM e concursos.",
  },
  {
    id: 10,
    imageSrc: "/assets/img/coursesCards/5.png",
    imageAlt: "image",
    rating: 4.2,
    ratingCount: 1991,
    title: "Raciocínio Lógico",
    lessonCount: 6,
    duration: 720,
    level: "Iniciante",
    authorImageSrc: "/assets/img/general/avatar-1.png",
    authorName: "Diego Rocha",
    originalPrice: 189,
    discountedPrice: 99,
    paid: true,
    category: "Matemática",
    state: "Em alta",
    viewStatus: "Médio",
    difficulty: "Média",
    desc: "Domine os fundamentos essenciais: operações, frações, porcentagens e regras básicas para resolver as primeiras questões do ENEM.",
  },
  {
    id: 11,
    imageSrc: "/assets/img/coursesCards/2.png",
    imageAlt: "image",
    rating: 5,
    ratingCount: 1991,
    title: "Leitura com propósito",
    lessonCount: 6,
    duration: 120,
    level: "Intermediário",
    authorImageSrc: "/assets/img/general/avatar-1.png",
    authorName: "José Silva",
    originalPrice: 209,
    discountedPrice: 119,
    paid: true,
    category: "Linguagens e Redação",
    state: "Popular",
    viewStatus: "Médio",
    difficulty: "Difícil",
    desc: "Desenvolva a análise crítica, interpretação de textos argumentativos e identificação de inferências para desafios do ENEM.",
  },
  {
    id: 12,
    imageSrc: "/assets/img/coursesCards/4.png",
    imageAlt: "image",
    rating: 5,
    ratingCount: 1991,
    title: "Matemática Básica",
    lessonCount: 6,
    duration: 1440,
    level: "Iniciante",
    authorImageSrc: "/assets/img/general/avatar-1.png",
    authorName: "Diego Rocha",
    originalPrice: 209,
    discountedPrice: 119,
    paid: true,
    category: "Matemática",
    state: "Popular",
    viewStatus: "Médio",
    difficulty: "Difícil",
    desc: "Domine os fundamentos essenciais: operações, frações, porcentagens e regras básicas para resolver as primeiras questões do ENEM.",
  },
  {
    id: 13,
    imageSrc: "/assets/img/coursesCards/10.png",
    authorImageSrc: "/assets/img/general/avatar-1.png",
    title: "História do Brasil",
    rating: 4.3,
    ratingCount: 1991,
    lessonCount: 6,
    duration: 1320,
    level: "Intermediário",
    originalPrice: 199,
    discountedPrice: 79,
    paid: true,
    category: "Ciências Humanas",
    state: "Popular",
    authorName: "Marcos Amaral",
    viewStatus: "Alto",
    difficulty: "Fácil",
    desc: "Aprofunde-se nos processos políticos, sociais e econômicos do Brasil imperial e início da república, focando nas questões do ENEM.",
  },
  {
    id: 14,
    imageSrc: "/assets/img/coursesCards/8.png",
    authorImageSrc: "/assets/img/general/avatar-1.png",
    title: "Química Fácil",
    rating: 4.7,
    ratingCount: 1991,
    lessonCount: 6,
    duration: 410,
    level: "Iniciante",
    originalPrice: 189,
    discountedPrice: 89,
    paid: true,
    category: "Ciências da Natureza ",
    authorName: "Simone Dos Santos",
    popular: true,
    new: true,
    bestSeller: true,
    state: "Em andamento",
    viewStatus: "Baixo",
    difficulty: "Fácil",
    desc: "Entenda os conceitos básicos: átomos, moléculas, propriedades da matéria e reações simples para começar a se familiarizar com a química.",
  },
  {
    id: 15,
    imageSrc: "/assets/img/coursesCards/6.png",
    authorImageSrc: "/assets/img/general/avatar-1.png",
    title: "Matemática Financeira",
    rating: 4.5,
    ratingCount: 1991,
    lessonCount: 6,
    duration: 1220,
    level: "Intermediário",
    originalPrice: 249,
    discountedPrice: 129,
    authorName: "Diego Rocha",
    paid: true,
    category: "Matemática",
    state: "Em alta",
    viewStatus: "Alto",
    difficulty: "Fácil",
    desc: "Aprofunde-se em juros compostos, séries de pagamentos e amortizações para resolver questões práticas do ENEM.",
  },
  {
    id: 16,
    imageSrc: "/assets/img/coursesCards/7.png",
    authorImageSrc: "/assets/img/general/avatar-1.png",
    title: "Social Media Marketing: Strategies for Growing Your Audience",
    rating: 4.2,
    ratingCount: 1991,
    lessonCount: 6,
    duration: 1020,
    level: "Iniciante",
    originalPrice: 179,
    discountedPrice: 99,
    authorName: "Simone dos Santos",
    paid: true,
    category: "Art",
    state: "Trending",
    viewStatus: "Good",
    difficulty: "Easy",
    desc: "Introductory course on web hosting, domain registration, and how you can easily publish and edit your website online.",
  },
  {
    id: 17,
    imageSrc: "/assets/img/coursesCards/10.png",
    authorImageSrc: "/assets/img/general/avatar-1.png",
    title: "Landscape Photography: Capturing Nature's Beauty",
    rating: 3.8,
    ratingCount: 1991,
    lessonCount: 6,
    duration: 250,
    level: "Intermediário",
    originalPrice: 169,
    discountedPrice: 79,
    paid: true,
    category: "Photography",
    authorName: "Jenny Wilson",
    state: "Popular",
    viewStatus: "Low",
    difficulty: "Meduium",
    desc: "Introductory course on web hosting, domain registration, and how you can easily publish and edit your website online.",
  },
  {
    id: 18,
    imageSrc: "/assets/img/coursesCards/1.png",
    imageAlt: "image",
    rating: 4.6,
    ratingCount: 1991,
    title: "Unity 3D Game Development: Create Your First FPS Game",
    lessonCount: 6,
    duration: 800,
    level: "Avançado",
    authorImageSrc: "/assets/img/general/avatar-1.png",
    authorName: "Albert Flores",
    originalPrice: 209,
    discountedPrice: 99,
    paid: true,
    category: "Animation",
    state: "Fetured",
    viewStatus: "Good",
    difficulty: "Meduium",
    desc: "Introductory course on web hosting, domain registration, and how you can easily publish and edit your website online.",
  },
  {
    id: 19,
    imageSrc: "/assets/img/coursesCards/2.png",
    imageAlt: "image",
    rating: 3.5,
    ratingCount: 1991,
    title: "Personal Finance and Investing: Securing Your Financial Future",
    lessonCount: 6,
    duration: 490,
    level: "Intermediário",
    authorImageSrc: "/assets/img/general/avatar-1.png",
    authorName: "Jacob Jones",
    originalPrice: 299,
    discountedPrice: 149,
    paid: true,
    category: "Writing",
    state: "Popular",
    viewStatus: "Great",
    difficulty: "Hard",
    desc: "Introductory course on web hosting, domain registration, and how you can easily publish and edit your website online.",
  },
  {
    id: 20,
    imageSrc: "/assets/img/coursesCards/4.png",
    imageAlt: "image",
    rating: 4.8,
    ratingCount: 1991,
    title: "Graphic Design Principles: Layouts, Typography, and Color Theory",
    lessonCount: 6,
    duration: 500,
    level: "Avançado",
    authorImageSrc: "/assets/img/general/avatar-1.png",
    authorName: "Robert Fox",
    originalPrice: 219,
    discountedPrice: 109,
    paid: true,
    category: "Photography",
    state: "Fetured",
    viewStatus: "Medium",
    difficulty: "Meduium",
    desc: "Introductory course on web hosting, domain registration, and how you can easily publish and edit your website online.",
  },
  {
    id: 21,
    imageSrc: "/assets/img/coursesCards/2.png",
    imageAlt: "image",
    rating: 3.9,
    ratingCount: 1991,
    title: "Node.js and Express.js: Creating Web Applications",
    lessonCount: 6,
    duration: 920,
    level: "Intermediário",
    authorImageSrc: "/assets/img/general/avatar-1.png",
    authorName: "Simone dos Santos",
    originalPrice: 199,
    discountedPrice: 89,
    paid: true,
    category: "Programming",
    state: "Popular",
    viewStatus: "Great",
    difficulty: "Hard",
    desc: "Introductory course on web hosting, domain registration, and how you can easily publish and edit your website online.",
  },
  {
    id: 22,
    imageSrc: "/assets/img/coursesCards/4.png",
    imageAlt: "image",
    rating: 4.2,
    ratingCount: 1991,
    title: "Android App Development: Building Dynamic Mobile Apps",
    lessonCount: 6,
    duration: 720,
    level: "Avançado",
    authorImageSrc: "/assets/img/general/avatar-1.png",
    authorName: "Jenny Wilson",
    originalPrice: 0,
    discountedPrice: 0,
    paid: false,
    category: "Writing",
    state: "Trending",
    viewStatus: "Medium",
    difficulty: "Meduium",
    desc: "Introductory course on web hosting, domain registration, and how you can easily publish and edit your website online.",
  },
  {
    id: 23,
    imageSrc: "/assets/img/coursesCards/6.png",
    imageAlt: "image",
    rating: 5,
    ratingCount: 1991,
    title: "Machine Learning in Python: Hands-On Projects",
    lessonCount: 6,
    duration: 120,
    level: "Intermediário",
    authorImageSrc: "/assets/img/general/avatar-1.png",
    authorName: "Albert Flores",
    originalPrice: 209,
    discountedPrice: 119,
    paid: true,
    category: "Design",
    state: "Popular",
    viewStatus: "Medium",
    difficulty: "Hard",
    desc: "Introductory course on web hosting, domain registration, and how you can easily publish and edit your website online.",
  },
  {
    id: 24,
    imageSrc: "/assets/img/coursesCards/6.png",
    imageAlt: "image",
    rating: 5,
    ratingCount: 1991,
    title: "Creative Writing Masterclass: Crafting Compelling Stories",
    lessonCount: 6,
    duration: 120,
    level: "Iniciante",
    authorImageSrc: "/assets/img/general/avatar-1.png",
    authorName: "Jacob Jones",
    originalPrice: 0,
    discountedPrice: 0,
    paid: false,
    category: "Design",
    state: "Popular",
    viewStatus: "Medium",
    difficulty: "Hard",
    desc: "Introductory course on web hosting, domain registration, and how you can easily publish and edit your website online.",
  },
  {
    id: 25,
    imageSrc: "/assets/img/coursesCards/2.png",
    authorImageSrc: "/assets/img/general/avatar-1.png",
    title: "Front-End Web Development: HTML, CSS, and JavaScript Fundamentals",
    rating: 4.3,
    ratingCount: 1991,
    lessonCount: 6,
    duration: 1320,
    level: "Intermediário",
    originalPrice: 199,
    discountedPrice: 79,
    paid: true,
    category: "Design",
    state: "Popular",
    authorName: "Robert Fox",
    viewStatus: "Good",
    difficulty: "Easy",
    desc: "Introductory course on web hosting, domain registration, and how you can easily publish and edit your website online.",
  },
  {
    id: 26,
    imageSrc: "/assets/img/coursesCards/10.png",
    authorImageSrc: "/assets/img/general/avatar-1.png",
    title: "Data Visualization with Tableau: Creating Informative Dashboards",
    rating: 4.7,
    ratingCount: 1991,
    lessonCount: 6,
    duration: 410,
    level: "Avançado",
    originalPrice: 0,
    discountedPrice: 0,
    paid: false,
    category: "Programming",
    authorName: "Albert Flores",
    popular: true,
    new: true,
    bestSeller: true,
    state: "Fetured",
    viewStatus: "Low",
    difficulty: "Easy",
    desc: "Introductory course on web hosting, domain registration, and how you can easily publish and edit your website online.",
  },
  {
    id: 27,
    imageSrc: "/assets/img/coursesCards/6.png",
    authorImageSrc: "/assets/img/general/avatar-1.png",
    title: "Digital Illustration: Bringing Your Imagination to Life",
    rating: 4.5,
    ratingCount: 1991,
    lessonCount: 6,
    duration: 1220,
    level: "Avançado",
    originalPrice: 249,
    discountedPrice: 129,
    authorName: "Simone dos Santos",
    paid: true,
    category: "Programming",
    state: "Trending",
    viewStatus: "Great",
    difficulty: "Easy",
    desc: "Introductory course on web hosting, domain registration, and how you can easily publish and edit your website online.",
  },
  {
    id: 28,
    imageSrc: "/assets/img/coursesCards/6.png",
    authorImageSrc: "/assets/img/general/avatar-1.png",
    title: "Java Enterprise Edition (EE): Building Scalable Web Applications",
    rating: 4.2,
    ratingCount: 1991,
    lessonCount: 6,
    duration: 1020,
    level: "Iniciante",
    originalPrice: 0,
    discountedPrice: 0,
    authorName: "Jenny Wilson",
    paid: false,
    category: "Art",
    state: "Trending",
    viewStatus: "Good",
    difficulty: "Easy",
    desc: "Introductory course on web hosting, domain registration, and how you can easily publish and edit your website online.",
  },
  {
    id: 29,
    imageSrc: "/assets/img/coursesCards/6.png",
    authorImageSrc: "/assets/img/general/avatar-1.png",
    title: "iOS App Design: User Interface and User Experience (UI/UX)",
    rating: 3.8,
    ratingCount: 1991,
    lessonCount: 6,
    duration: 250,
    level: "Avançado",
    originalPrice: 169,
    discountedPrice: 79,
    paid: true,
    category: "Photography",
    authorName: "Jacob Jones",
    state: "Popular",
    viewStatus: "Low",
    difficulty: "Meduium",
    desc: "Introductory course on web hosting, domain registration, and how you can easily publish and edit your website online.",
  },
  {
    id: 30,
    imageSrc: "/assets/img/coursesCards/9.png",
    imageAlt: "image",
    rating: 4.6,
    ratingCount: 1991,
    title: "Introduction to Artificial Intelligence: Concepts and Applications",
    lessonCount: 6,
    duration: 800,
    level: "Iniciante",
    authorImageSrc: "/assets/img/general/avatar-1.png",
    authorName: "Albert Flores",
    originalPrice: 0,
    discountedPrice: 0,
    paid: false,
    category: "Animation",
    state: "Fetured",
    viewStatus: "Good",
    difficulty: "Meduium",
    desc: "Introductory course on web hosting, domain registration, and how you can easily publish and edit your website online.",
  },
  {
    id: 31,
    imageSrc: "/assets/img/coursesCards/7.png",
    imageAlt: "image",
    rating: 3.5,
    ratingCount: 1991,
    title: "Video Editing with Adobe Premiere Pro: From Shoot to Final Cut",
    lessonCount: 6,
    duration: 490,
    level: "Avançado",
    authorImageSrc: "/assets/img/general/avatar-1.png",
    authorName: "Albert Flores",
    originalPrice: 299,
    discountedPrice: 149,
    paid: true,
    category: "Writing",
    state: "Popular",
    viewStatus: "Great",
    difficulty: "Hard",
    desc: "Introductory course on web hosting, domain registration, and how you can easily publish and edit your website online.",
  },
  {
    id: 32,
    imageSrc: "/assets/img/coursesCards/9.png",
    imageAlt: "image",
    rating: 4.8,
    ratingCount: 1991,
    title: "Cybersecurity Essentials: Protecting Your Digital World",
    lessonCount: 6,
    duration: 500,
    level: "Iniciante",
    authorImageSrc: "/assets/img/general/avatar-1.png",
    authorName: "Simone dos Santos",
    originalPrice: 0,
    discountedPrice: 0,
    paid: false,
    category: "Photography",
    state: "Fetured",
    viewStatus: "Medium",
    difficulty: "Meduium",
    desc: "Introductory course on web hosting, domain registration, and how you can easily publish and edit your website online.",
  },
  {
    id: 33,
    imageSrc: "/assets/img/coursesCards/10.png",
    imageAlt: "image",
    rating: 3.9,
    ratingCount: 1991,
    title: "Photography Post-Processing: Enhancing and Editing Your Images",
    lessonCount: 6,
    duration: 920,
    level: "Avançado",
    authorImageSrc: "/assets/img/general/avatar-1.png",
    authorName: "Jenny Wilson",
    originalPrice: 199,
    discountedPrice: 89,
    paid: true,
    category: "Programming",
    state: "Popular",
    viewStatus: "Great",
    difficulty: "Hard",
    desc: "Introductory course on web hosting, domain registration, and how you can easily publish and edit your website online.",
  },
  {
    id: 34,
    imageSrc: "/assets/img/coursesCards/12.png",
    imageAlt: "image",
    rating: 4.2,
    ratingCount: 1991,
    title: "Full-Stack Web Development with MERN: MongoDB, Express.js, React, Node.js",
    lessonCount: 6,
    duration: 720,
    level: "Iniciante",
    authorImageSrc: "/assets/img/general/avatar-1.png",
    authorName: "Jacob Jones",
    originalPrice: 0,
    discountedPrice: 0,
    paid: false,
    category: "Writing",
    state: "Trending",
    viewStatus: "Medium",
    difficulty: "Meduium",
    desc: "Introductory course on web hosting, domain registration, and how you can easily publish and edit your website online.",
  },
  {
    id: 35,
    imageSrc: "/assets/img/coursesCards/6.png",
    imageAlt: "image",
    rating: 5,
    ratingCount: 1991,
    title: "Content Marketing Strategy: Engaging Your Target Audience",
    lessonCount: 6,
    duration: 120,
    level: "Intermediário",
    authorImageSrc: "/assets/img/general/avatar-1.png",
    authorName: "Albert Flores",
    originalPrice: 209,
    discountedPrice: 119,
    paid: true,
    category: "Design",
    state: "Popular",
    viewStatus: "Medium",
    difficulty: "Hard",
    desc: "Introductory course on web hosting, domain registration, and how you can easily publish and edit your website online.",
  },
  {
    id: 36,
    imageSrc: "/assets/img/coursesCards/1.png",
    imageAlt: "image",
    rating: 5,
    ratingCount: 1991,
    title: "Wildlife Photography: Capturing Animals in Their Natural Habitat",
    lessonCount: 6,
    duration: 120,
    level: "Avançado",
    authorImageSrc: "/assets/img/general/avatar-1.png",
    authorName: "Albert Flores",
    originalPrice: 0,
    discountedPrice: 0,
    paid: false,
    category: "Design",
    state: "Popular",
    viewStatus: "Medium",
    difficulty: "Hard",
    desc: "Introductory course on web hosting, domain registration, and how you can easily publish and edit your website online.",
  },
  {
    id: 37,
    imageSrc: "/assets/img/coursesCards/12.png",
    authorImageSrc: "/assets/img/general/avatar-1.png",
    title: "Game Design and Storytelling: Creating Immersive Gaming Experiences",
    rating: 4.3,
    ratingCount: 1991,
    lessonCount: 6,
    duration: 1320,
    level: "Intermediário",
    originalPrice: 199,
    discountedPrice: 79,
    paid: true,
    category: "Design",
    state: "Popular",
    authorName: "Simone dos Santos",
    viewStatus: "Good",
    difficulty: "Easy",
    desc: "Introductory course on web hosting, domain registration, and how you can easily publish and edit your website online.",
  },
  {
    id: 38,
    imageSrc: "/assets/img/coursesCards/1.png",
    authorImageSrc: "/assets/img/general/avatar-1.png",
    title: "Financial Planning for Retirement: Securing Your Golden Years",
    rating: 4.7,
    ratingCount: 1991,
    lessonCount: 6,
    duration: 410,
    level: "Avançado",
    originalPrice: 0,
    discountedPrice: 0,
    paid: false,
    category: "Programming",
    authorName: "Jenny Wilson",
    popular: true,
    new: true,
    bestSeller: true,
    state: "Fetured",
    viewStatus: "Low",
    difficulty: "Easy",
    desc: "Introductory course on web hosting, domain registration, and how you can easily publish and edit your website online.",
  },
  {
    id: 39,
    imageSrc: "/assets/img/coursesCards/11.png",
    authorImageSrc: "/assets/img/general/avatar-1.png",
    title: "User Interface (UI) Design Fundamentals: Principles and Best Practices",
    rating: 4.5,
    ratingCount: 1991,
    lessonCount: 6,
    duration: 1220,
    level: "Intermediário",
    originalPrice: 249,
    discountedPrice: 129,
    authorName: "Robert Fox",
    paid: true,
    category: "Programming",
    state: "Trending",
    viewStatus: "Great",
    difficulty: "Easy",
    desc: "Introductory course on web hosting, domain registration, and how you can easily publish and edit your website online.",
  },
  {
    id: 40,
    imageSrc: "/assets/img/coursesCards/8.png",
    authorImageSrc: "/assets/img/general/avatar-1.png",
    title: "Python Web Scraping: Extracting Data from Websites",
    rating: 4.2,
    ratingCount: 1991,
    lessonCount: 6,
    duration: 1020,
    level: "Iniciante",
    originalPrice: 179,
    discountedPrice: 99,
    authorName: "Jacob Jones",
    paid: true,
    category: "Art",
    state: "Trending",
    viewStatus: "Good",
    difficulty: "Easy",
    desc: "Introductory course on web hosting, domain registration, and how you can easily publish and edit your website online.",
  },
  {
    id: 41,
    imageSrc: "/assets/img/coursesCards/3.png",
    authorImageSrc: "/assets/img/general/avatar-1.png",
    title: "Digital Marketing Analytics: Measuring and Optimizing Campaigns",
    rating: 3.8,
    ratingCount: 1991,
    lessonCount: 6,
    duration: 250,
    level: "Avançado",
    originalPrice: 169,
    discountedPrice: 79,
    paid: true,
    category: "Photography",
    authorName: "Jacob Jones",
    state: "Popular",
    viewStatus: "Low",
    difficulty: "Meduium",
    desc: "Introductory course on web hosting, domain registration, and how you can easily publish and edit your website online.",
  },
  {
    id: 42,
    imageSrc: "/assets/img/coursesCards/11.png",
    imageAlt: "image",
    rating: 4.6,
    ratingCount: 1991,
    title: "Adobe Illustrator Mastery: Creating Vector Artwork",
    lessonCount: 6,
    duration: 800,
    level: "Iniciante",
    authorImageSrc: "/assets/img/general/avatar-1.png",
    authorName: "Simone dos Santos",
    originalPrice: 209,
    discountedPrice: 99,
    paid: true,
    category: "Animation",
    state: "Fetured",
    viewStatus: "Good",
    difficulty: "Meduium",
    desc: "Introductory course on web hosting, domain registration, and how you can easily publish and edit your website online.",
  },
  {
    id: 43,
    imageSrc: "/assets/img/coursesCards/1.png",
    imageAlt: "image",
    rating: 3.5,
    ratingCount: 1991,
    title: "Ruby on Rails: Building Web Applications with Ease",
    lessonCount: 6,
    duration: 490,
    level: "Intermediário",
    authorImageSrc: "/assets/img/general/avatar-1.png",
    authorName: "Jenny Wilson",
    originalPrice: 299,
    discountedPrice: 149,
    paid: true,
    category: "Writing",
    state: "Popular",
    viewStatus: "Great",
    difficulty: "Hard",
    desc: "Introductory course on web hosting, domain registration, and how you can easily publish and edit your website online.",
  },
];

export const catagories = [
  "Todos Categories",
  "Animation",
  "Design",
  "Photography",
  "Art",
  "Programming",
  "Writing",
];

export const courseStates = ["Todos", "Fetured", "Popular", "Trending"];

export const viewStatus = ["Todos", "Great", "Good", "Medium", "Low"];

export const resentCourses = [
  {
    id: 1,
    imageSrc: "/assets/img/dashboard/recent-courses/1.png",
    title: "Complete Python Bootcamp From Zero to Hero in Python",
    author: "Ali Tufan",
    authorImg: `/assets/img/general/avatar-1.png`,
    lessonCount: 6,
    duration: 236,
  },
  {
    id: 2,
    imageSrc: "/assets/img/dashboard/recent-courses/2.png",
    title: "The Ultimate Drawing Course Iniciante to Advanced",
    author: "Ali Tufan",
    authorImg: `/assets/img/general/avatar-1.png`,
    lessonCount: 6,
    duration: 236,
  },
  {
    id: 3,
    imageSrc: "/assets/img/dashboard/recent-courses/3.png",
    title: "Instagram Marketing 2021: Complete Guide To Instagram Growth",
    author: "Ali Tufan",
    authorImg: `/assets/img/general/avatar-1.png`,
    lessonCount: 6,
    duration: 236,
  },
];

export const allCategories = [
  "Todos",
  "Design",
  "Programming",
  "Art",
  "Photography",
  "Animation",

  "Writing",
];

export const difficulty = ["Todos", "Hard", "Meduium", "Easy"];

export const tags = [
  { id: 1, href: "#", name: "Courses" },
  { id: 2, href: "#", name: "Learn" },
  { id: 3, href: "#", name: "Online" },
  { id: 4, href: "#", name: "Education" },
  { id: 5, href: "#", name: "LMS" },
  { id: 6, href: "#", name: "Training" },
];

export const instractorNames = [
  { id: 1, title: "Simone dos Santos" },
  { id: 2, title: "Jenny Wilson" },
  { id: 3, title: "Robert Fox" },
  { id: 4, title: "Jacob Jones" },
  { id: 5, title: "Albert Flores" },
];

export const levels = [
  { id: 2, title: "Iniciante" },
  { id: 3, title: "Intermediário" },
  { id: 4, title: "Avançado" },
];
export const prices = [
  { id: 1, title: "Todos" },
  { id: 2, title: "Free" },
  { id: 3, title: "Paid" },
];

export const rating = [
  { id: 1, star: 5, text: "4.5 & up", range: [4.5, 5] },
  { id: 2, star: 5, text: "4.0 & up", range: [4, 5] },
  { id: 3, star: 5, text: "3.5 & up", range: [3.5, 5] },
  { id: 4, star: 5, text: "3.0 & up", range: [3, 5] },
];

export const categories = [
  { id: 1, title: "Art" },
  { id: 2, title: "Animation" },
  { id: 3, title: "Design" },
  { id: 4, title: "Photography" },
  { id: 5, title: "Programming" },

  { id: 6, title: "Writing" },
];

export const duration = [
  { id: 1, title: "Less than 4 hours", range: [0, 239] },
  { id: 2, title: "4 - 7 hours", range: [240, 419] },
  { id: 3, title: "7 - 18 hours", range: [420, 1079] },
  { id: 4, title: "20 + Hours", range: [1080, 5000] },
];

export const sortingOptions = [
  "Default",
  "Rating (asc)",
  "Rating (dsc)",
  "Price (asc)",
  "Price (dsc)",
  "Duration (asc)",
  "Duration (dsc)",
];
